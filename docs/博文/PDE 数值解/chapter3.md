...
* 线性双曲型偏微分方程 (PDEs)：输运方程 (transport equation)、波动方程 (wave equation)、特征线 (characteristic lines)。
* 线性混合方程：对流扩散方程 (advection-diffusion equation)、拟线性双曲型方程 (quasi-linear hyperbolic equation)。
* 双曲守恒律 (hyperbolic conservation laws)：通量 (flux)、总变差减小 (TVD)、激波 (shock wave)、Godunov 方法、ENO / WENO 格式、黎曼求解器 (Riemann solvers)。
...
其中 $A_{i}$, $B\in\mathbb{R}^{p\times p}$，$\psi:\mathbb{R}^{n}\times\mathbb{R}^{+}\rightarrow\mathbb{R}^{p}$。如果对于任意 $\alpha=(\alpha_{1},\dots,\alpha_{p})\in\mathbb{R}^{p}\setminus\{0\}$，$A=\sum_{i=1}^{p}\alpha_{i}A_{i}$ 是实可对角化的，则称该 PDE 为双曲型的。
...
所以 $w_{i}(x_{i}(t),t)$ 是常数。因此对于任意 $x, t$，有 $w_{i}(x,t)=w_{i}(x-\lambda_{i}t,0)$。假设 $R=(\xi^{1},\dots,\xi^{p})$，则 $w_{i}=\eta^{i}u$。我们称 $w_{i}$ 为特征 $(\lambda_{i},\xi^{i})$ 的黎曼不变量 (Riemann invariant)。那么

$$
u(x,t)=Rw(x,t)=\sum\xi^{i}w_{i}(x,t)=\sum\xi^{i}w_{i}(x-\lambda_{i}t,0)=\sum_{i=1}^{p}\xi^{i}\eta^{i}u_{0}(x-\lambda_{i}t).
$$

**定义 1.** 对于区域 $\Omega\subseteq\mathbb{R}\times\mathbb{R}^{+}$，其**依赖域 (dependence domain)** 定义为
...
**TVD (总变差减小) 性质:** $TV(U^{m+1})\le TV(U^{m})$，其中 $TV(U^{m})=\sum_{j=0}^{N-1}|U_{j+1}^{m}-U_{j}^{m}|\approx\int|\frac{\partial u}{\partial x}|dx$ 是离散总变差。
该性质等价于单调格式或极值点数非增。

可以证明迎风格式和 LF 格式满足 TVD 性质。所以它们是 $L^{2}$-稳定的（或 $L^{1}$-稳定的）。

**定理 1 (Godnov 定理).** 任何单调（或 TVD）线性格式至多是一阶精度的。
...
对于给定的 $k_{0}$，令 $k$ 接近 $k_{0}$，$k=k_{0}+\Delta k, \Delta k\ll k_{0}$，则
...
由于 $1-\mu^{2}\ge0$，总是 $w_{0}\le ak$（太小）。如果 $k$ 增加，则 $|w_{0}-ak|$ 增加，这是糟糕的。
...
**修正方程分析:**
所有数值格式都可以通过泰勒级数转化为无限项的常微分方程 (ODEs)，即 $\tilde{u}_{t}=\sum_{m=0}^{\infty}a_{m}\partial_{x}^{m}\tilde{u}$。
令 $u=e^{i(kx-wt)}$，
...
令 $v=u_{t}$, $w=-au_{x}$，向量形式为 $(\begin{smallmatrix}v\\ w\end{smallmatrix})_{t}+(\begin{smallmatrix}0&a\\ a&0\end{smallmatrix})(\begin{smallmatrix}v\\ w\end{smallmatrix})_{x}=0$。
...
所以 $\begin{cases}U_{j}^{0}=u_{0}(x_{j}),\\ U_{j}^{1}=U_{j}^{0}+\tau v_{0}(x_{j})+\frac{1}{2}\mu^{2}\delta_{x}^{2}U_{j}^{0}.\end{cases}$。

对于 Von Neumann 分析，$\lambda_{k}^{2}-2\lambda_{k}+1=\lambda_{k}\mu^{2}(e^{ikh}+e^{-ikh}-2)$
...
$w_{0}=-\frac{\arg(\lambda_{k})}{\tau}=\pm ak(1-\frac{1}{24}(1-\mu^{2})k^{2}h^{2}+\dots)$。
...
并且 $w_{0}=-\frac{1}{\tau}\arg(\lambda_{k})=\pm ak[1-\frac{1}{24}(1+(12\theta-1)\mu^{2})k^{2}h^{2}+\dots]$。
...
假设 $P^{-1}AP=\text{diag}(\lambda_{1},\dots,\lambda_{n})$。对于所有 $\lambda_{1}\le\dots\le\lambda_{n}$，令 $\Lambda^{\pm}=\text{diag}(\lambda_{1}^{\pm},\dots,\lambda_{n}^{\pm})$。
$\lambda_{k}^{\pm}=\frac{1}{2}(|\lambda_{k}|\pm\lambda_{k})$。
$\lambda_{k}=\lambda_{k}^{+}-\lambda_{k}^{-}$, $|\lambda_{k}|=\lambda_{k}^{+}+\lambda_{k}^{-}$, $\Lambda=\Lambda^{+}-\Lambda^{-}$, $|\Lambda|=\Lambda^{+}+\Lambda^{-}$。
$A^{\pm}=P\Lambda^{\pm}P^{-1}$。则 $A=A^{+}-A^{-}$。
...
我们考虑二维问题 $u_{tt}=a^{2}(u_{xx}+u_{yy})$。
...
**注:** 双曲型 PDEs 与抛物型 PDEs 有相同的麻烦。随着维度增加，对于显式格式，稳定性条件将变为 $\mu\le\frac{1}{\sqrt{d}}$；对于隐式格式，单步线性系统的系数矩阵将越来越复杂。
...
通过 $u_{j}^{m+1}=[u+\tau u_{t}+\frac{1}{2}\tau^{2}u_{tt}]_{j}^{m}+\mathcal{O}(\tau^{3})=[u-a\tau u_{x}+c\tau u_{xx}+\frac{a^{2}\tau^{2}}{2}u_{xx}]_{j}^{m}+\mathcal{O}(c\tau^{2}+\tau^{3})$，
我们有
...
稳定性条件是
$L^{2}: (c+\frac{1}{2}a^{2}\tau)\frac{\tau}{h^{2}}\le\frac{1}{2}, \tau\le\frac{2c+a^{2}\tau}{a^{2}}$ (后者已满足)。
...
它可以看作是修正的中心差分。稳定性条件为
$L^{2}: (c+\frac{1}{2}ah)\frac{\tau}{h^{2}}\le\frac{1}{2}, \tau\le\frac{2c+ah}{a^{2}}$ (已满足)。
$L^{\infty}: (c+\frac{1}{2}ah)\frac{\tau}{h^{2}}\le\frac{1}{2}, h\le\frac{2c+ah}{a}$ (已满足)。
...
$L^2$: $\lambda_k = \frac{1-\mu(1-\cos kh)-i\frac{\nu}{2}\sin kh}{1+\mu(1-\cos kh)+i\frac{\nu}{2}\sin kh}$，无条件稳定。

$L^\infty$: $(1+\mu)U_j^{m+1} = (1-\mu)U_j^m + (\frac{\mu}{2}-\frac{\nu}{4})U_{j+1}^m + (\frac{\mu}{2}+\frac{\nu}{4})U_{j-1}^m + \dots$
条件: $\mu\le1, h\le\frac{2c}{a}$ ($Pe\le2$)。

### 4.2 算子分裂 / 乘积公式 / Trotter 公式

我们考虑方程形式 $u_{t}=Au+Bu$，其中 A, B 是微分算子。例如，
...
为简单起见，假设 A, B 与时间无关。（如果它们与时间有关，我们可以使用 Dyson 级数代替泰勒级数。）所以 $\partial_{t}Au=Au_{t}, \partial_{t}Bu=Bu_{t} \Rightarrow \partial_{t}^{k}u=(A+B)^{k}u$。
...
$U_{2k}(\tau)=U_{2k-2}^{2}(s_{k}\tau)U_{2k-2}((1-4s_{k})\tau)U_{2k-2}^{2}(s_{k}\tau)$，其中 $s_{k}=1/(4-4^{\frac{1}{2k-1}})$，
...
所以 $u_{t}+a(u)u_{x}=0$, $a(u)=f^{\prime}(u)$。
...
例如，我们考虑无粘 Burgers 方程 (一维 Euler 方程) $u_{t}+uu_{x}=0$。
...
**定义 2 (弱解).** 对于方程 $u_{t}+f^{\prime}(u)u_{x}=0$，其弱解定义为
(1) $\forall x_{L}$, $\int_{x_{L}}^{x_{R}}(u_{t}+f^{\prime}(u)u_{x})dx=0 \Rightarrow \frac{d}{dt}\int_{x_{L}}^{x_{R}}u(x,t)dx+f(u(x_{R},t))-f(u(x_{L},t))=0$。
(2) 测试函数 $\phi\in C^{1}(\mathbb{R}^{2})$ 具有紧支集，$\int_{\mathbb{R}^{+}}\int_{\mathbb{R}}\phi(u_{t}+f^{\prime}(u)u_{x})dxdt=0 \Rightarrow \int_{\mathbb{R}^{+}}\int_{\mathbb{R}}(\phi_{t}u+\phi_{x}f(u))dxdt+\int_{\mathbb{R}}\phi(x,0)u(x,0)dx=0$。
这里 (1) 和 (2) 是等价的。
...
图为：在线 $t=2x+1$ 上，两类特征线相交。

如果 $u_{0}(x)=\begin{cases}1,&x<0,\\ 0,&x\ge0\end{cases}$，那么它的弱解是 $u(x,t)=\begin{cases}1,&x<\frac{t}{2},\\ 0,&x>\frac{t}{2},\\ \frac{1}{2},&x=\frac{t}{2}.\end{cases}$

如果 $u_{0}(x)=\begin{cases}0,&x<0,\\ 1,&x\ge0\end{cases}$，那么它的弱解可以是 $u(x,t)=\begin{cases}0,&x<\frac{t}{2},\\ 1,&x>\frac{t}{2},\\ \frac{1}{2},&x=\frac{t}{2}.\end{cases}$。但这还不够好，因为它不满足熵解。
...
令 $u(x,t)=\begin{cases}0,&x\le0,\\ \frac{x}{t},&0<x<t,\\ 1,&x\ge t.\end{cases}$。
这是一个满足熵条件的解。它被称为自相似解。

**定理 2.** 对于方程 $u_{t}+(f(u))_{x}=0$，$u_0(x) = \begin{cases} u_L, & x<0 \\ u_R, & x>0 \end{cases}$。
如果 f(u) 是凸的，那么我们有
...
**(1) 非守恒格式:** 考虑迎风格式 $U_{j}^{m+1}=U_{j}^{m}-\frac{\tau}{h}(U_{j}^{m}(U_{j}^{m}-U_{j-1}^{m}))$，$U_{j}^{0}=\begin{cases}1,&j<0,\\ 0,&j\ge0.\end{cases}$。
...
令 $\overline{U}_{j}^{m}=\frac{1}{h}\int_{x_{j-\frac{1}{2}}}^{x_{j+\frac{1}{2}}}u(x,t_{m})dx$, $\overline{f}_{j+\frac{1}{2}}^{m+\frac{1}{2}}=\frac{1}{\tau}\int_{t_{m}}^{t_{m+1}}f(u(x_{j+\frac{1}{2}},t))dt$，
则 $\overline{U}_{j}^{m+1}=\overline{U}_{j}^{m}-\frac{\tau}{h}(\overline{f}_{j+\frac{1}{2}}^{m+\frac{1}{2}}-\overline{f}_{j-\frac{1}{2}}^{m+\frac{1}{2}})$
...
离散守恒律为 $\sum_{j\in\mathbb{Z}}U_{j}^{m+1}=\sum_{j\in\mathbb{Z}}U_{j}^{m}$。

**(a) Roe 迎风格式:**
令 $a_{j+\frac{1}{2}}^{m}=\begin{cases}\frac{f(U_{j+1}^{m})-f(U_{j}^{m})}{U_{j+1}^{m}-U_{j}^{m}},&U_{j}^{m}\ne U_{j+1}^{m},\\ 0,&U_{j}^{m}=U_{j+1}^{m},\end{cases}$
$F_{j+\frac{1}{2}}^{m+\frac{1}{2}}=\begin{cases}f(U_{j}^{m}),&a_{j+\frac{1}{2}}^{m}\ge0,\\ f(U_{j+1}^{m}),&a_{j+\frac{1}{2}}^{m}<0.\end{cases}$
如果 $a_{j+\frac{1}{2}}^{m}\ge0$ 总是成立，则 $U_{j}^{m+1}=U_{j}^{m}-\frac{\tau}{h}(f(U_{j}^{m})-f(U_{j-1}^{m}))$。
...
**(c) Lax-Wendroff 格式**
通过 $U_{j}^{m+1}=[u+\tau u_{t}+\frac{1}{2}\tau^{2}u_{tt}]_{j}^{m}+\mathcal{O}(\tau^{3})=[u-\tau(f(u))_{x}+\frac{1}{2}\tau^{2}(f^{\prime}(u)(f(u))_{x}]_{j}^{m}+\mathcal{O}(\tau^{3})$，
...
我们令 $F_{j+\frac{1}{2}}^{m+\frac{1}{2}}=f(u_{*}(U_{j}^{m},U_{j+1}^{m}))$。
$u_{*}(U_{j}^{m},U_{j+1}^{m}) = w(0, U_j^m, U_{j+1}^m)$，
其中 $w(\frac{x}{t},u_{L},u_{R})$ 是黎曼问题 $u(x,0)=\begin{cases}u_{L},&x<0,\\ u_{R},&x>0.\end{cases}$ 的解。
...
**定义 4.** 对于 $U_{j}^{m+1}=U_{j}^{m}-\frac{\tau}{h}(F_{j+\frac{1}{2}}^{m+\frac{1}{2}}-F_{j-\frac{1}{2}}^{m+\frac{1}{2}}):=H(U_{j-1}^{m},U_{j}^{m},U_{j+1}^{m})$。
如果 $\forall v_{i}\ge u_{i}$, $H(v_{1},v_{2},v_{3})\ge H(u_{1},u_{2},u_{3})$，则称格式是**单调**的。
我们将 H 的单调性表示为 $H(\uparrow,\uparrow,\uparrow)$。

我们有以下充分条件：
单调 $\Leftarrow \frac{\partial H}{\partial u_{i}}\ge0$。
$=U_{j}^{m+1}=U_{j}^{m}-\frac{\tau}{h}(f(U_{j}^{m},U_{j+1}^{m})-f(U_{j-1}^{m},U_{j}^{m}))$，
$\frac{\tau}{h}(\partial_{u_{1}}f(u_{1},u_{2})-\partial_{u_{2}}f(u_{1},u_{2}))\le1$。
$\Leftarrow |\nu|\le Q(\nu)\le1$，这里 $|\nu|\le1$ 是 CFL 条件。
...
Lax-Friedrichs: $Q=1$.
Roe: $Q=|a_{j+1/2}^m|$.
Godunov: $Q=\frac{f(U_j)+f(U_{j+1})-2f(u_*(U_j, U_{j+1}))}{h/\tau(U_{j+1}-U_j)}$.
Lax-Wendroff: $Q = (\frac{\tau}{h} a_{j+1/2}^m)^2$.

然后我们回顾一些性质：
$L_{1}$-压缩: $||U^{m+1}||_{L_{1}} \le ||U^{m}||_{L_{1}}$。
TVD: $||U^{m+1}||_{TV} \le ||U^{m}||_{TV}$，其中 $||U^{m}||_{TV}=\sum_{j}|U_{j+1}^{m}-U_{j}^{m}|$。
保单调: 如果 $U_{j}^{m}\le U_{j+1}^{m}$，则 $U_{j}^{m+1}\le U_{j+1}^{m+1}$。
TVB: $||U^{m+1}||_{TV} \le (1+\tau)||U^{m}||_{TV}$。

可以证明：
单调 $\Rightarrow L_{1}$-压缩 $\Rightarrow$ TVD $\Rightarrow$ 保单调 (如果线性 $\Leftrightarrow$ 单调)。
TVD $\Rightarrow$ TVB。

在所有格式（Roe, LW, LF, Godunov）中，LW 仅满足 TVB，其他三种格式满足单调性。

**定理 3 (Godunov 定理).** 线性单调格式至多是一阶精度的。
（当 $f(u)$ 关于 u 线性时，如果 $U_{j}^{m+1}=\sum c_{i}U_{j+i}^{m}$，即如果 $m+1$ 步的近似解可以表示为 $m$ 步近似解的线性组合，则它是线性格式。）

**注:** Lax-Wendroff 格式是二阶的，所以它不是单调的并且会产生振荡。
