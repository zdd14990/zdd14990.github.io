...
$$
f_Y(y)=\frac{d}{dy}F_Y(y)=\frac{1}{\pi(1+y^2)},\quad y\in\mathbb{R}.
$$
...
$$
\sigma(X)=\left\{A\in \mathcal{B}(\mathbb{R}):A=-A\right\}
$$

(2).

$$
\sigma(X)=\left\{X^{-1}(B):B\in \mathcal{B}\left(\left[-\frac{1}{2},\frac{1}{2}\right]\right)\right\}.
$$
...
$$
\lim_{x\to+\infty}
\mathbb{E}\left[|X|\mathbf{1}_{\{|X|< x\}}\right]= \mathbb{E}[|X|]<\infty.
$$
...
\mathbb{E}\left[|X|\mathbf{1}_{\{|X|\geq x\}}\right]
= \lim_{x\to+\infty}
\mathbb{E}[|X|]-\mathbb{E}\left[|X|\mathbf{1}_{\{|X|< x\}}\right]
=0.
$$
...
$$
   \mathbb{E}[X_n]\xrightarrow[n\to\infty]{}+\infty,\qquad\liminf_{n\to\infty}\frac{\operatorname{var}(X_n)}{\mathbb{E}[X_n]^2}=0.
$$

   Prove that

$$
   X_n \xrightarrow[n\to\infty]{} +\infty  \quad \text{a.s.}
$$
...
$$
   \liminf_{n\to\infty}  \frac{  \sum_{j=1}^n\sum_{k=1}^n \mathbb{P}(A_j\cap A_k) }{   \left(\sum_{k=1}^n \mathbb{P}(A_k)\right)^2   }   =   1
$$
...
$$
\frac{\mathrm{Var}(X_{n_j})}{m_{n_j}^2}\to 0.
$$

Set $X_\infty=\lim_{j\to\infty}X_{n_j}$. We have

$$
\left\{X_\infty<\infty\right\}=\bigcup_{M=1}^{\infty}\left\{X_\infty\le M\right\}.
$$
...
$$
\begin{aligned}
    \mathrm{P}\left(X_{n_j}\le M\right)&\le \mathrm{P}\left(|X_{n_j}-m_{n_j}|\ge m_{n_j}-M\right)\\
    &\le \frac{\mathrm{Var}(X_{n_j})}{(m_{n_j}-M)^2}\to 0.
\end{aligned}
$$
...
$$
\mathrm{P}\left(X_\infty<\infty\right)=\lim_{n_j\to\infty}\mathrm{P}\left(X_{n_j}<\infty\right)=0.
$$
